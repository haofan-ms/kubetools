...
install_helm_app() {
    ...
    if [[ -z $namespace ]]; then
        ssh -t -i $identityFile \
        $userName@$connectionIP \
        "helm install azs-ecs/$appName --generate-name" --set nodeSelector.'beta\.kubernetes\.io/os'=linux"
    else
        ssh -t -i $identityFile \
        $userName@$connectionIP \
        "helm install azs-ecs/$appName --namespace $namespace --generate-name" --set nodeSelector.'beta\.kubernetes\.io/os'=linux"
    fi
