...

{
    ...
    while [ $i -le $MAX_INGRESS_COUNT ]; do
        ...
        ssh -t -i $IDENTITY_FILE $USER_NAME@$MASTER_IP "helm install $ingressName azs-ecs/nginx-ingress --namespace $NAMESPACE_NAME --set controller.replicaCount=2,controller.nodeSelector.'beta\.kubernetes\.io/os'=linux,defaultBackend.nodeSelector.'beta\.kubernetes\.io/os'=linux || true"
